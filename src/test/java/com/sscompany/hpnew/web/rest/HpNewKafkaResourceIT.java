...
import com.sscompany.hpnew.config.KafkaProperties;
...
class HpNewKafkaResourceIT {
    ...
    void setup() {
        ...
        HpNewKafkaResource kafkaResource = new HpNewKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/hp-new-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/hp-new-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
